...
class ColorSpace {
  ...
  static #subParse(
    ...
    const isRef = cs instanceof Ref;

    const cachedCS = isRef && globalColorSpaceCache.getByRef(cs);
    if (cachedCS) {
      console.log(cs, cachedCS); // eslint-disable-line no-console
      return cachedCS;
    }
    const parsedCS = this.#parse(
      cs,
    ...
    if (isRef) {
      globalColorSpaceCache.set(/* name = */ null, cs, parsedCS);
    }
    return parsedCS;
  }
