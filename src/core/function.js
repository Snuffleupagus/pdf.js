...
import { FormatError, info, shadow } from "../shared/util.js";
...
class PDFFunction {
  ...
  static parse({ xref, fn }) {
    const dict = fn.dict || fn;
    const typeNum = dict.get("FunctionType");

    switch (typeNum) {
      case 0:
        return this.constructSampled({ xref, fn, dict });
      case 1:
        break;
      case 2:
        return this.constructInterpolated({ xref, dict });
      case 3:
        return this.constructStiched({ xref, dict });
      case 4:
        return this.constructPostScript({ xref, fn, dict });
    }
    throw new FormatError("Unknown type of function");
  }

  static parseArray({ xref, fnObj }) {
    if (!Array.isArray(fnObj)) {
      // not an array -- parsing as regular function
      return this.parse({ xref, fn: fnObj });
    }

    const fnArray = [];
    for (const fn of fnObj) {
      fnArray.push(this.parse({ xref, fn: xref.fetchIfRef(fn) }));
    }
  ...
  static constructSampled({ xref, fn, dict }) {
  ...
  static constructInterpolated({ xref, dict }) {
  ...
  static constructStiched({ xref, dict }) {
    ...
    for (const fn of dict.get("Functions")) {
      fns.push(this.parse({ xref, fn: xref.fetchIfRef(fn) }));
    }
  ...
  static constructPostScript({ xref, fn, dict }) {
    const domain = toNumberArray(dict.getArray("Domain"));
